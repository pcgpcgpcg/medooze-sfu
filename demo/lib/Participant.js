...
class Participant
{
        ...
        */
		...
		this.transport.on("outgoingtrack",(track,stream)=>{
			...
			this.transport.on("incomingtrack",(track,stream)=>{
				...
				//貌似不需要处理了
				console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~incomingtrack " + stream.getId());
			...
			return {
                                ...
                                candidates: endpoint.getLocalCandidates(),
                                capabilities:this.room.capabilities
	...
	addStream(stream) {		
		...
		if(!outgoingStream){
			...
			//add to the list
			this.outgoingStreams.set(outgoingStream.id,outgoingStream);
		}

		for(const track of stream.getTracks()){
			let outgoingStreamTrack = outgoingStream.getTrack(track.getId());
			if(!outgoingStreamTrack) {			
				outgoingStream.createTrack(track);
				track.once("stopped",()=>{
        	                        //send ended event
                	                const pcns =this.tm.namespace("medooze::pc::0");

                        	        console.log("------12126--------------------------" + stream.getTracks().length + " " + track.getId());
                               		pcns.event("removedtrack",{
	...
	{
		this.debug("stop");
		console.log("*******"+this.incomingStreams.size);
